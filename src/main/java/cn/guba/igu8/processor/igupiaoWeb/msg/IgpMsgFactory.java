...
import cn.guba.igu8.processor.igupiaoWeb.msg.beans.*;
...
public class IgpMsgFactory {
    ...
    public void updateLiverMsg(Cookie cookie, int uid) {
        ...
        for (Teacher teacher : igpTeacherList) {
            ...
            if (uidNone) {
                uid = 5 * 100000 + random.nextInt(99999);
            }
            ...
            // 如果获取失败，再次获取一次
            if (liverMsg == null) {
                liverMsg = getLiverMsgLimit(cookie, uid + 1, pfId, maxId);
            }
    ...
    public IgpWebMsgBean[] getWebMsgArr(Cookie cookie, int uid, int teacherPfId, long lastTime) {
        ...
        // 如果获取失败，再次获取一次
        if (liverMsg == null) {
            liverMsg = getLiverMsg(cookie, uid + 1, teacherPfId, lastTime);
        }
        if (liverMsg != null) {
